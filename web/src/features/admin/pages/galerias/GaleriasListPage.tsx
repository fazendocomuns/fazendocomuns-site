...
import { useGalerias, useGaleriaMutations } from '@/features/admin/api/adminEntities'
import { usePaginatedList } from '@/features/admin/hooks/usePaginatedList'

export function GaleriasListPage() {
  const router = useRouter()
  const { data: galerias = [], isLoading } = useGalerias()
  const { remove, toggleStatus } = useGaleriaMutations()
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader
        title="Galerias"
        description="Gerencie as galerias de fotos exibidas no site."
        createHref="/admin/galerias/novo"
        createLabel="Nova galeria"
      />
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          actionLabel="Adicionar galeria"
          onAction={() => router.push('/admin/galerias/novo')}
        />
      ) : (
            ...
            <TableBody>
              {paginated.map((item) => (
                <TableRow key={item.id} className="group">
                  <TableCell>
                    <Link
                      to={`/admin/galerias/${item.id}/editar`}
                  ...
                  <TableCell>
                    <RowActions
                      editHref={`/admin/galerias/${item.id}/editar`}
                      onDelete={() => {
                        setDeleteId(item.id)
